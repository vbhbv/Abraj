...
async def safe_edit_message(query, text: str, reply_markup=None, parse_mode=ParseMode.MARKDOWN_V2):
    """
    بديل آمن لـ query.edit_message_text: يحاول التعديل العادي أولاً.
    لو كانت الرسالة الحالية رسالة صورة (مثل خريطة الفلك المُولَّدة)، تليجرام يرفض
    تعديل نصها برسالة "There is no text in the message to edit"، فنتراجع تلقائيًا
    ونرسل رسالة نصية جديدة بدل ما نفشل بصمت أو نوقف الهاندلر بالكامل.
    """
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=parse_mode)
    except BadRequest as e:
        err = str(e).lower()
        if "there is no text in the message to edit" in err or "no text in the message" in err:
            await query.message.reply_text(text, reply_markup=reply_markup, parse_mode=parse_mode)
        elif "message is not modified" in err:
            pass  # نفس النص والأزرار بالضبط، لا داعي لأي إجراء
        else:
            raise
...
async def synastry_trigger_workflow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not saved_profile:
        ...
        await safe_edit_message(query, err_profile, parse_mode=ParseMode.MARKDOWN_V2)
    ...
    await safe_edit_message(query, step1_msg, parse_mode=ParseMode.MARKDOWN_V2)
    return P2_YEAR
...
async def p2_knows_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "p2_knows_true":
        ...
        await safe_edit_message(query, ask_time, parse_mode=ParseMode.MARKDOWN_V2)
        return P2_TIME
    else:
        ...
        await safe_edit_message(query, ask_loc, parse_mode=ParseMode.MARKDOWN_V2)
        return P2_LOCATION
...
async def electional_trigger_workflow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_edit_message(query, trigger_msg, parse_mode=ParseMode.MARKDOWN_V2)
...
async def khira_start_from_menu(query: Any, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit_message(query, welcome_khira, reply_markup=khira_engine.get_main_keyboard(), parse_mode=ParseMode.MARKDOWN_V2)
...
async def astrology_trigger_workflow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if saved_profile:
        ...
        await safe_edit_message(query, found_msg, parse_mode=ParseMode.MARKDOWN_V2)
    ...
    await safe_edit_message(query, start_astro, parse_mode=ParseMode.MARKDOWN_V2)
    return YEAR
...
async def p_knows_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "knows_true":
        ...
        await safe_edit_message(query, ask_t, parse_mode=ParseMode.MARKDOWN_V2)
        return TIME
    else:
...
async def handle_menu_clicks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_home":
        ...
        await safe_edit_message(query, home_msg, reply_markup=get_start_keyboard(), parse_mode=ParseMode.MARKDOWN_V2)
        return
        ...
        await safe_edit_message(query, reset_msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للقائمة", callback_data="main_home")]]), parse_mode=ParseMode.MARKDOWN_V2)
        return
        ...
        await safe_edit_message(query, result_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ العودة لقائمة الخيرة", callback_data="khira_back")]]), parse_mode=ParseMode.MARKDOWN_V2)
        return
        ...
        await safe_edit_message(query, rules_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ العودة لقائمة الخيرة", callback_data="khira_back")]]), parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    try:
        if data == "menu_horoscope_daily":
            ...
            await safe_edit_message(query, horoscope_report, reply_markup=astrology_back_markup, parse_mode=ParseMode.MARKDOWN_V2)
            ...
            await safe_edit_message(query, forecast_report, reply_markup=astrology_back_markup, parse_mode=ParseMode.MARKDOWN_V2)
            ...
            if len(complete_analysis) > 4000:
                ...
                await safe_edit_message(query, complete_analysis, reply_markup=astrology_back_markup, parse_mode=ParseMode.MARKDOWN_V2)
