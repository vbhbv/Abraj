...
class AstrologyChartDrawer:
    def __init__(self, size: int = 900):
        ...
        # 9. نظام الحلقات الأربع الاحترافي الشامل (Radii Rings)
    ...
    def _get_coordinates(self, angle_deg: float, radius: float) -> tuple:
        ...
        # فلكياً: الطالع (ASC) يقع دائماً على يسار الشاشة تماماً (الزاوية 180 درجة هندسياً)
        # لتثبيت الطالع عند زاوية 180، نقوم بطرح الدرجة الحقيقية من 180
    ...
    def _format_degree(self, num_deg: float) -> str:
        """7. تحويل الدرجة العشرية إلى صيغة فلكية كلاسيكية مثل 24°46"""
        degrees = int(num_deg)
    ...
    def _resolve_collisions(self, planets_angles: List[Dict[str, Any]], min_dist: float = 6.0) -> List[Dict[str, Any]]:
        """
        3 + 10. خوارزمية ذكية مخصصة لمنع تداخل الكواكب (Planet Collision Resolver)
        تعتمد على الفحص الدائري والتحريك التكيفي للزوايا المتقاربة جداً
        """
        # ترتيب الكواكب حسب زواياها الطولية لتسهيل الفحص المتتالي
        sorted_planets = sorted(planets_angles, key=lambda x: x['orig_angle'])
        n = len(sorted_planets)
        if n <= 1:
            return sorted_planets

        # تشغيل خوارزمية الدفع المغناطيسي التكراري (Iterative Relaxation)
        for _ in range(5):  # 5 دورات فحص كافية لفك التجمعات (Stelliums)
            for i in range(n):
                ...
                # حساب المسافة الزاوية مع مراعاة قفزة الـ 360 درجة
                ...
                if diff < min_dist:
                    ...
                    # دفع الكوكبين في اتجاهين متعاكسين لتوسيع الفجوة
    ...
    def generate_chart_svg(self, chart_data: Any) -> str:
        """توليد كود الـ SVG النقي بناءً على البيانات والمحاور الحقيقية المحدثة"""
        
        # 1. استخدام الدرجة الحقيقية للطالع والـ MC للرسم الدقيق
        asc_deg = getattr(chart_data, 'ascendant_degree', 0.0)
        mc_deg = getattr(chart_data, 'midheaven_degree', 0.0)
        
        svg_elements = []
        
        # ترويسة الـ SVG مع دمج حقول دقة الرندرة الجيومترية والنصية (geometricPrecision)
        svg_elements.append(
        ...
        # 9. رسم الدوائر الهيكلية للحلقات الأربع الاحترافية
        ...
        # 4. رسم قطاعات الأبراج الـ 12 بالتساوي من درجة الصفر الكونية (الحمل) وتدويرها نسبة للطالع
        for i in range(12):
            ...
            # إسقاط رموز الأبراج في مركز الحلقة الخارجية بدقة
        ...
        # 6. رسم خطوط البيوت الـ 12 الفلكية الدقيقة وتسمية الأوتاد الأربعة (ASC, MC, DSC, IC)
        if hasattr(chart_data, 'houses'):
            # مصفوفة الأوتاد ومقابلاتها الحسابية الهندسية
            ic_deg = (mc_deg + 180.0) % 360
            dsc_deg = (asc_deg + 180.0) % 360
            ...
            for h_num, h_deg in chart_data.houses.items():
                ...
                # تضخيم خطوط الأوتاد الرئيسية باللون والأبعاد
                ...
                # طباعة اسم الوتد أو رقم البيت في نهاية مسار الخط
                if is_axis:
        ...
        # 5. رسم خطوط الاتصالات الداخلية بقوة تعتيم (Opacity) تعتمد ديناميكياً على الـ Orb الحقيقي
        if hasattr(chart_data, 'aspects'):
            for aspect in chart_data.aspects:
                # 2. قراءة خط الطول الحقيقي للكواكب مباشرة من نموذجك المحدث (longitude)
                ...
                # حساب الـ Opacity ديناميكياً: كلما قل الـ Orb اقترب التعمية من 1.0 (خط ساطع)
                orb = getattr(aspect, 'orb', 0.0)
                max_orb = 8.0  # القيمة الافتراضية القصوى للمجال الفلكي
                opacity = max(0.15, round(1.0 - (orb / max_orb), 2))
        ...
        # 3 + 10. تشغيل محرك فض النزاعات والتداخل للكواكب المتقاربة
        raw_planets_data = []
        for p_name, p_data in chart_data.planets.items():
            if p_name in self.PLANET_UNICODE:
                # 2. الاعتماد الكامل على حقل الـ longitude المتوفر في الـ Engine الخاص بك
                p_abs_deg = p_data.longitude
                raw_planets_data.append({
                    'name': p_name,
                    'orig_angle': p_abs_deg - asc_deg,
                    'curr_angle': p_abs_deg - asc_deg,
                    'display_deg': p_data.longitude % 30 # الدرجة الفردية داخل البرج ذاته
                })

        # فك الاشتباك والتداخل الحركي
        resolved_planets = self._resolve_collisions(raw_planets_data, min_dist=7.5)

        # 7. رسم رموز الكواكب مع إضافة الدرجات والدقائق الفلكية بالكامل (مثل ☉ 24°46')
        for p in resolved_planets:
            sym = self.PLANET_UNICODE[p['name']]
            
            # وضع نقطة دقيقة عند الدرجة الأصلية الخام غير المزاحة
            ...
            # رسم رمز الكوكب عند الزاوية الجديدة (المزاحة برمجياً لمنع التداخل)
            ...
            # طباعة الدرجة والدقيقة مصفوفة تحت الرمز مباشرة بجمالية برمجية دقيقة
        ...
        # إضافة اسم السيرفر التجاري في قلب الدائرة لضمان الهوية المؤسسية للمنتج
